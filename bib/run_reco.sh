...
INPUT_FILE="${1:?Usage: run_reco.sh <input.edm4hep.root> <output.edm4hep.root> <benchmarks_path> <detector_geom> [num_threads]}"
OUTPUT_FILE="${2:?}"
MUCOLL_BENCHMARKS="${3:?}"
...
# Isolated scratch cwd: Pandora needs its settings in cwd and reco_histograms.root
# is written to cwd, so every worker must run in its own directory.
SCRATCH="/tmp/bib_reco_$$_${RANDOM}"
...
CMD=(k4run "${MUCOLL_CONFIG}/${MUCOLL_CONFIG_NAME}/reco_steer.py"
...
echo "RECO step complete: ${OUTPUT_FILE}"
